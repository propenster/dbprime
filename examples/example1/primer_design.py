...
try:
    ...
    raise ModuleNotFoundError(
        "primer3-py is required for this script. Please install it via 'pip install primer3-py' or 'conda install -c bioconda primer3-py'."
    )


def design_primer(
    left_flank, right_flank, product_size_range=(100, 300), num_primers=5
):
...
def run_pairwise_primer_design(args):
    ...
    candidates_df = utils.parse_samfile(
        sam_path,
        query_seqs,
        ref_seqs,
    ...
    if args.design_primers:
        ...
        for c in candidates_df.itertuples():
            ...
            if primer_info:
                ...
                logging.info(
                    f"Primer design successful for candidate {c.Index}: {primer_info}"
                )
...
def run_primer_design(
    ...
    logging.info(f"Alignment completed. See output SAM file at {sam_path}")
    logging.info("Loading sequences")
    ref_seqs = utils.load_fasta(
        reference, COMPRESSED=True if reference.endswith(".gz") else False
    )
    query_seqs = utils.load_fasta(fasta_path, COMPRESSED=compressed)
    logging.info("Parsing SAM file for indels")
    candidates_df = utils.parse_samfile(
        sam_path, query_seqs, ref_seqs, min_indel_length=min_indel_length, flank=flank
    )
    logging.info(f"Found {len(candidates_df)} indel candidates.")
    ...
    if design_primers:
        ...
        tsv_output_path, json_output_path = utils.export_primer_results_to_file(
            candidates_df, output_file, sample_output
        )
...
def run_multi_primer_design(args):
    ...
    if not fasta_files:
        logging.error(
            f"No FASTA files found in the queries directory '{args.queries}'."
        )
    ...
    with ThreadPoolExecutor(max_workers=args.threads) as executor:
        ...
        for future in tqdm(
            ...
            sample_name, tsv_output_path, json_output_path, candidates_df = (
                future.result()
            )
